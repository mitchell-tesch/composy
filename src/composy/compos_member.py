...
class ComposMember:
    ...
    def get_utilisation_factors(self) -> result.UtilisationFactors:
        ...
        for factor in [
            results_enums.UtilisationFactor.CONST_MOMENT,
    ...
    def get_station_results(self, result_type: results_enums.RESULT_TYPES) -> result.StationResults:
        ...
        for station_index in range(0, self._num_stations):
            station_results.append(self._compos_auto.Result(self._name, str(result_type), station_index))
        return result.StationResults(result_type, station_results)

    def get_station_result(self, result_type: results_enums.RESULT_TYPES, station_index: int) -> result.StationResult:
    ...
    def get_max_result(self, result_type: results_enums.RESULT_TYPES) -> result.MaxResult:
        ...
        max_result, station_index = self._compos_auto.MaxResult(self._name, str(result_type), station_index)
    ...
    def get_min_result(self, result_type: results_enums.RESULT_TYPES) -> result.MinResult:
        ...
        min_result, station_index = self._compos_auto.MinResult(self._name, str(result_type), station_index)
        return result.MinResult(result_type, station_index, min_result)

    def get_trans_rebar_properties(self, property_type: results_enums.TransRebarProp) -> result.TransRebarProperties:
        ...
        for rebar_index in range(0, self._num_trans_rebar):
            property_values.append(self._compos_auto.TranRebarProp(self._name, str(property_type), rebar_index))
        return result.TransRebarProperties(property_type, property_values)

    def get_trans_rebar_property(
        self, property_type: results_enums.TransRebarProp, rebar_index: int
    ) -> result.TransRebarProperty:
        ...
        if not self._results_available:
            raise ComposyError('Member has no results available.')
        property_value: float = self._compos_auto.TranRebarProp(self._name, str(property_type), rebar_index)
    ...
    def _get_station_positions(self):
        ...
        for station_index in range(0, self._num_stations):
            self._stations.append(
                self._compos_auto.Result(self._name, str(results_enums.ResultStations.SECTION_DIST), station_index)
            )
