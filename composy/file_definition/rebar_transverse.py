...
class RebarTransverse():
    ...
    def __repr__(self) -> str:
        if self.is_user_defined:
            return f"REBAR_TRANSVERSE({self.type},{self.start},{self.end},{self.diameter},\
                {self.spacing},{self.cover},{self.grade})"
        return f"REBAR_TRANSVERSE({self.type})"

    def __str__(self) -> str:
        if self.is_user_defined:
            return f"REBAR_TRANSVERSE,{self.member_name},{self.type},{self.start},{self.end},{self.diameter},\
                {self.spacing},{self.cover},{self.grade}"
        return f"REBAR_TRANSVERSE,{self.member_name},{self.type}"
